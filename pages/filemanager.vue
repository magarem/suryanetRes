  ...
  <div class="flex w-full h-full text-gray-200 antialiased">
    ...
    >
          ...
          <ul>
            <li v-for="item in navigationItems" :key="item.key" class="text-sm">
              <div
                ...
                class="flex items-center justify-between px-2 py-2.5 hover:bg-gray-700 rounded-lg transition-colors cursor-pointer"
              ...
              <ul
                v-if="!isSidebarCollapsed && item.isOpen && item.children"
                class="pl-1 mt-1 space-y-0"
              >
                <li v-for="child in item.children" :key="child.key">
                  ...
                  >
                    <i
                      :class="child.icon"
                      class="w-2 text-center mr-3 text-gray-400"
                    ></i>
      ...
      <div>
        ...
        <div class="mt-8 p-2">
          ...
          <p v-if="!isSidebarCollapsed" class="text-xs text-gray-400 mt-2">
        ...
        <div class="mt-4 pt-4 border-t border-gray-700">
          <button
            ...
            class="w-full flex items-center p-2.5 hover:bg-gray-700 rounded-lg transition-colors"
            :class="isSidebarCollapsed ? 'justify-center' : ''"
          >
            <i
              class="fas"
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
      ...
      <div class="flex-1 p-4 overflow-y-auto">
        ...
        <div
          v-if="!searchQuery"
          class="flex items-center text-sm text-gray-400 mb-4"
        >
          <div
        ...
        <div v-else class="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-2">
          ...
          <div v-for="item in displayContent" :key="item.id">
            ...
            <div
              ...
              class="bg-gray-800 rounded-lg p-4 flex flex-col items-center justify-center text-center hover:bg-gray-700/50 transition-all cursor-pointer group h-52 relative"
            >
              <div @click.stop class="absolute top-2 right-2 z-20">
                <button
                  @click="showMenu($event, item)"
                  class="text-white opacity-0 group-hover:opacity-100 hover:text-blue-400 p-2 rounded-full hover:bg-gray-700/80 transition-opacity"
                >
                  <i class="fas fa-ellipsis-v"></i>
                </button>
              </div>
              <i class="fas fa-folder text-yellow-400 text-5xl mb-3"></i>
              <h3 class="font-medium text-white truncate w-full">
                {{ item.name }}
              </h3>
            ...
            >
              ...
              <div
                v-else
                class="flex items-center justify-center h-24 rounded-md mb-3 text-4xl flex-shrink-0"
    ...
    <Dialog
      ...
      modal
      :header="appConfig.dialogs.newFolder.header"
      :style="{ width: '25rem' }"
      :pt="dialogStyles"
    >
      <div class="flex flex-col gap-2 mt-4">
        <label for="foldername" class="font-semibold">{{
          appConfig.dialogs.newFolder.label
        }}</label>
        <InputText
          id="foldername"
      ...
      <template #footer>
        <Button
          :label="appConfig.buttons.cancel"
          text
          severity="secondary"
    ...
    <Dialog
      ...
      modal
      :header="appConfig.dialogs.rename.header"
      :style="{ width: '25rem' }"
      :pt="dialogStyles"
    >
      <div class="flex flex-col gap-2 mt-4">
        <label for="newitemname" class="font-semibold">{{
          appConfig.dialogs.rename.label
        }}</label>
        <InputText
          id="newitemname"
          v-model="newItemName"
    ...
    <Dialog
      ...
      modal
      :header="appConfig.dialogs.move.header"
      :style="{ width: '25rem' }"
      :pt="dialogStyles"
    >
      <div class="mt-4">
    ...
    <Dialog
      ...
      modal
      :header="appConfig.dialogs.upload.header"
      :style="{ width: '40rem' }"
      :pt="dialogStyles"
    >
      <div class="mt-4">
    ...
    <Dialog
      ...
      modal
      :header="appConfig.dialogs.delete.header"
      :style="{ width: '25rem' }"
      :pt="dialogStyles"
    >
      ...
      <template #footer>
        ...
        />
        <Button
          :label="appConfig.buttons.delete"
          severity="danger"
          @click="deleteItem"
        />
      </template>
    ...
    >
      <div class="flex items-center justify-center p-4 bg-gray-900">
        <img
          v-if="imageToView"
          :src="getImageUrl(imageToView.url)"
...
const { data: fileSystem, pending, error, refresh } = await useFetch(
  "/api/files"
);
...
const showMenu = (event, item) => {
  ...
  const baseActions = [
    {
      label: appConfig.menuActions.move,
      icon: "pi pi-folder-open",
      command: () => confirmMoveItem(item)
    },
    {
      label: appConfig.menuActions.rename,
      icon: "pi pi-pencil",
      command: () => confirmRenameItem(item)
    },
    { separator: true },
    {
      label: appConfig.menuActions.delete,
      icon: "pi pi-trash",
  ...
  if (item.type === "file") {
    const fileActions = [
      {
        label: appConfig.menuActions.download,
        icon: "pi pi-download",
        command: () => downloadFile(item)
      },
      {
        label: appConfig.menuActions.copyPath,
        icon: "pi pi-link",
        command: () => copyPathToClipboard(item)
      },
      {
        label: appConfig.menuActions.copyFile,
        icon: "pi pi-clone",
        command: () => copyItem(item)
      }
    ];

    if (item.preview === "image" || item.iconClass?.includes("fa-file-pdf")) {
      fileActions.unshift({
        label: appConfig.menuActions.view,
        icon: "pi pi-eye",
        command: () => viewFile(item)
      });
    }
  ...
  try {
    await $fetch("/api/folders", {
      ...
      body: {
        name: name,
        parentPath:
          currentFolder.value.id === "root" ? "" : currentFolder.value.id
      }
    });
    ...
    const detail =
      err.data?.error || appConfig.toasts.createFolder.error.detail;
...
//   },
  dialogs: {
    ...
    upload: {
      header: "Upload Files",
      empty: "Drag and drop files to here to upload."
    },
    delete: {
  ...
  toasts: {
    ...
    copyItem: {
      ...
      error: {
        severity: "error",
        summary: "Error",
        detail: "Could not copy file."
      },
      warn: {
        severity: "warn",
        summary: "Info",
        detail: "Copying folders is not yet supported."
      }
    },
    deleteItem: {
      success: {
