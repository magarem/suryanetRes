  ...
  <div class="flex h-[calc(100vh-8rem)] bg-gray-800 rounded-xl overflow-hidden shadow-lg">
    ...
    <div class="w-1/3 border-r border-gray-700 flex flex-col">
      ...
      <div class="p-4 border-b border-gray-700 flex justify-between items-center flex-shrink-0">
        <div class="relative">
            <select v-model="selectedMailbox" @change="onFolderChange" class="bg-gray-700 text-white text-xl font-bold rounded-lg py-2 pl-4 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors">
    ...
    <div class="w-2/3 flex flex-col">
      <template v-if="selectedMessage">
        ...
        <!-- Reply Box -->
        <div v-if="selectedMailbox === 'inbox'" class="p-4 border-t border-gray-700 flex-shrink-0">
...
async function fetchEmails() {
  ...
  const emailPromises = emails.value.map(async email => {
    return {
      ...email,
      avatar: await checkImageExists(`api/uploads/adm/avatar.png?${Date.now()}`)    
    };
  });
